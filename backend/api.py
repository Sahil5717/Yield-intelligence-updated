...
# ─── CORS ──────────────────────────────────────────────────────────────
# Configure via CORS_ORIGINS env var as a comma-separated list, e.g.
#   CORS_ORIGINS=https://yi.example.com,https://staging.example.com
# For local dev, defaults include Vite's 5173. The "*" wildcard is no
# longer allowed — explicit origins only, which is required anyway once
# credentials/Authorization flow through the browser.
...
app.add_middleware(
    ...
    allow_credentials=False,   # we use Bearer tokens in Authorization header, not cookies
