...
# Guard: if running in production (RAILWAY_ENVIRONMENT or YI_ENV=production),
# refuse to boot with the default dev secret. Catches the #1 deploy mistake.
if SECRET_KEY == "yield-intelligence-dev-secret-change-in-production" and (
    os.environ.get("RAILWAY_ENVIRONMENT") or os.environ.get("YI_ENV") == "production"
):
    raise RuntimeError(
